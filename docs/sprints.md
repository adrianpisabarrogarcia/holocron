...
* **Mover páginas arrastrando (drag-and-drop)**: `PageTreeSidebar` ahora soporta arrastrar una página sobre otra para reparentarla (se agrega como su última subpágina), o soltarla sobre una zona "Mover a la raíz" que aparece mientras se arrastra para devolverla al nivel superior. Usa la misma API nativa de HTML5 Drag and Drop que ya usaba el tablero Kanban para las tarjetas (`draggable`, `onDragStart/Over/Drop`), sin agregar ninguna librería. Reutiliza el mismo endpoint `PATCH` existente enviando solo `parentPageId` — no se agregó ningún endpoint nuevo. El reordenamiento fino entre hermanos (posición exacta dentro del mismo padre) queda fuera de este alcance.
* **Fix: el drag no soltaba la página**: el cursor mostraba "mover" pero el drop no aplicaba nada. Primer intento: se corrigió que `dragover` solo llamara `preventDefault()` cuando el drop ya era válido (dejaba el cursor en estado inconsistente sobre destinos rechazados sin feedback), y se unificó el click de selección con los handlers de drag en el mismo elemento. Como el problema persistió y la API nativa de HTML5 Drag and Drop es notoriamente inconsistente entre navegadores (y no hay forma de verificarla interactivamente en este entorno de desarrollo), se reemplazó por completo por una implementación propia basada en eventos de mouse (`mousedown`/`mousemove`/`mouseup` sobre `window`, con un umbral de 6px antes de considerar que empezó un arrastre real, y resolución del destino vía `document.elementFromPoint` + atributos `data-drop-target`). Esto saca de la ecuación cualquier comportamiento específico del navegador o de anidar elementos interactivos dentro de un `draggable`, dando control total sobre cuándo se considera "arrastre" vs "click simple".
