...
### Páginas (Wiki) por Proyecto

Se agregó un espacio tipo Confluence dentro de cada proyecto, independiente del tablero de tareas, para documentación libre.

* **Modelo de datos**: nuevos modelos `Page` (árbol jerárquico vía auto-relación `parentPage`/`subPages` con cascade delete, igual que `Folder`) y `PageVersion` (historial). Cada página pertenece a un `Project`.
* **Editor**: Markdown fuente con vista previa en vivo (`PageEditor`), renderizado con `react-markdown` + `remark-gfm` (soporte GFM: tablas, strikethrough) y estilos vía `@tailwindcss/typography`.
* **Permisos**: reutiliza los roles de proyecto existentes sin esquema nuevo — Viewer solo lectura, Contributor/Manager/Admin pueden crear, editar, reparentar y borrar.
* **Versionado básico**: cada edición de título/contenido snapshotea el estado anterior en `PageVersion`. El historial (`PageVersionHistory`) permite previsualizar y restaurar una versión pasada; restaurar también snapshotea el estado previo a la restauración.
* **Validaciones**: se impide reparentar una página bajo sí misma o uno de sus propios descendientes (detección de ciclos), y se impide asignar como padre una página de otro proyecto.
* **API**: nuevo feature `apps/api/src/features/pages/` con endpoints CRUD, listado/lectura de versiones y restauración, protegidos con el mismo `requireProjectAccess` que el resto de la plataforma.
* **UI**: nueva sección "Páginas" en la navegación lateral de cada workspace/proyecto (`/workspace/:slug/pages`), con árbol de páginas (mismo patrón flatten+indent que el selector de carpetas de administración) y deep-link por `?page=<id>`.
* **Editor enriquecido con toolbar**: se agregó una barra de herramientas (`MarkdownToolbar`) al editor de páginas con acciones de formato (negrita, cursiva, tachado, títulos H1-H3, listas, cita, enlace, código en línea/bloque, tabla, línea horizontal), operando sobre la selección del `<textarea>` mediante helpers puros en `markdownEditing.ts`. Incluye atajos de teclado (Ctrl/Cmd+B/I/K, Tab para indentar) y soporte de pegar/soltar imágenes directamente en el editor (reutiliza `compressImageToWebp` y el endpoint de subida ya usados por el editor de tareas), insertando automáticamente la sintaxis `![alt](url)`.
* **Fix: persistencia del proyecto seleccionado al recargar**: el selector de proyecto superior (`App.tsx`) no sincronizaba el proyecto activo con la URL — `handleProjectChange` nunca escribía `?project=`, así que cualquier recarga volvía siempre al primer proyecto de la lista. Primera iteración: se escribe `?project=<id>` al cambiar de proyecto y se restaura desde ese parámetro. `PagesPage` incluye `?project=` junto a `?page=` al abrir una página, y se agregó un botón "Copiar enlace" en `PageViewer` (mismo patrón que en `TaskModal`) para compartir el link directo a una página concreta.
* **Fix: persistencia global (no solo por URL)**: la navegación por el sidebar cambia de ruta sin arrastrar el query string, así que recargar en una página distinta a la del último cambio seguía perdiendo la selección. Se agregó persistencia en `localStorage` (`useBoardStore.ts`, clave por workspace) como respaldo: `loadBoard` ahora prioriza el proyecto en memoria, luego el guardado en `localStorage` para el workspace activo, y recién después el primero de la lista. Un `?project=` explícito en la URL (link compartido) sigue teniendo prioridad sobre lo guardado.
* **Mover páginas arrastrando (drag-and-drop)**: `PageTreeSidebar` soporta arrastrar una página sobre otra para reparentarla, o soltarla sobre una zona "Mover a la raíz" que aparece mientras se arrastra para devolverla al nivel superior.
* **Fix: el drag no soltaba la página**: el cursor mostraba "mover" pero el drop no aplicaba nada. Primer intento con HTML5 Drag and Drop nativo (mismo patrón que las tarjetas del Kanban): se corrigió que `dragover` solo llamara `preventDefault()` cuando el drop ya era válido, y se unificó el click de selección con los handlers de drag en el mismo elemento. Como el problema persistió y la API nativa es notoriamente inconsistente entre navegadores (sin forma de verificarla interactivamente en este entorno de desarrollo), se reemplazó por completo por una implementación propia basada en eventos de mouse (`mousedown`/`mousemove`/`mouseup` sobre `window`, umbral de 6px antes de considerar que empezó un arrastre real, resolución del destino vía `document.elementFromPoint` + atributos `data-drop-target`).
* **Reordenamiento fino entre hermanos**: además de reparentar, ahora se puede reordenar una página entre sus hermanas (antes/después) arrastrándola al tercio superior o inferior de otra fila (el tercio central sigue significando "anidar como hija"). Nuevo endpoint `PUT /api/projects/:projectId/pages/reorder` (mismo patrón de "sincronizar lista completa con posiciones" que ya usaba `PUT /api/projects/:projectId/columns` para las columnas del tablero): recibe `{ parentPageId, orderedPageIds }`, valida ciclos si el padre cambia, y reescribe `position = índice` para toda la lista en una sola transacción. El cliente calcula la lista completa de hermanos (insertando la página arrastrada en el índice correcto) y la envía entera — no hay reindexado parcial en el servidor.
